...
const EditShelterScreen = () => {
  ...
  const { data: shelter, isLoading, isError } = useQuery({
  ...
  const mutation = useMutation({
    mutationFn: (data: ShelterFormValues) => shelterService.updateById(shelterId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['shelter', shelterId] });
  ...
  if (isError || !shelter) {
        Alert.alert('Erro', 'Erro ao carregar dados do abrigo.')
        return null;
  }

  return (
    <View style={{ flex:1 }}>
      <ShelterForm initialValues={shelter} onSubmit={handleSubmit} />
    </View>
  );
};
